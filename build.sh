...
build() {
    pkgrel() { _tar_u "PlatinumSrc_Engine_${PLATPATH}" psrc; }
    buildrel "${1}" "${PLATDESC}" "${@:2}"
    pkgrel() { _tar_u "PlatinumSrc_Engine_${PLATPATH32}" psrc; }
    buildrel "${1}" "${PLATDESC32}" "${@:2}" M32=y
    #pkgrel() { _zip_u "PlatinumSrc_Engine_Windows_x86_64" psrc.exe; }
    #buildrel "${1}" "Windows x86_64" "${@:2}" CROSS=win32
    pkgrel() { _zip_u "PlatinumSrc_Engine_Windows_i686" psrc.exe; }
    buildrel "${1}" "Windows i686" "${@:2}" CROSS=win32 M32=y CC='wine i686-w64-mingw32-gcc' inc.null=NUL
}
buildmod "engine"

build() {
    pkgrel() { _tar_u "PlatinumSrc_Server_${PLATPATH}" psrc-server; }
    buildrel "${1}" "${PLATDESC}" "${@:2}"
    pkgrel() { _tar_u "PlatinumSrc_Server_${PLATPATH32}" psrc-server; }
    buildrel "${1}" "${PLATDESC32}" "${@:2}" M32=y
    #pkgrel() { _zip_u "PlatinumSrc_Server_Windows_x86_64" psrc-server.exe; }
    #buildrel "${1}" "Windows x86_64" "${@:2}" CROSS=win32
    pkgrel() { _zip_u "PlatinumSrc_Server_Windows_i686" psrc-server.exe; }
    buildrel "${1}" "Windows i686" "${@:2}" CROSS=win32 M32=y CC='wine i686-w64-mingw32-gcc' inc.null=NUL
}
buildmod "server"
